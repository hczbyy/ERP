    ...
    create({ products, priceField, initialItems = [], stockMap = null }) {
      ...
      function renderRow(it, idx) {
        ...
        tr.innerHTML = `
          ...
          <td><input type="number" min="0" step="0.01" class="num" value="${it.qty ?? ""}" placeholder="数量"></td>
      ...
      el.isValid = () => {
        ...
        for (const it of items) {
          if (!(it.qty > 0)) return { ok: false, msg: "商品数量必须大于 0" };
          ...
          try {
            ...
            tbody.querySelectorAll("[data-act]").forEach((b) => {
              b.onclick = () => {
                ...
                ({ detail: showDetail, edit: () => openForm(o), approve: () => approve(o), receive: () => receive(o), cancel: () => cancel(o), del: () => del(o) })[b.dataset.act]();
              };
            });
