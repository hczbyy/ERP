    ...
    orders: {
      render(el) {
          ...
          try {
            ...
            tbody.querySelectorAll("[data-act]").forEach((b) => {
              b.onclick = () => {
                ...
                ({ detail: () => showDetail(o.id), edit: () => openForm(o), approve: () => approve(o), ship: () => ship(o), cancel: () => cancel(o), del: () => del(o) })[b.dataset.act]();
              };
            });
        ...
        async function openForm(row) {
          ...
          if (row) {
            try { row = await API.get(`/api/sales/orders/${row.id}`); }
