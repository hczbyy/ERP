...
(function () {
  ...
  function openSettle({ title, apiPath, row, statusText, noLabel }) {
    ...
    foot.querySelector('[data-act="yes"]').onclick = async () => {
      ...
      const isReceipt = apiPath.includes("/receipts");
      try {
        const r = await API.post(apiPath, {
          [isReceipt ? "receivable_id" : "payable_id"]: row.id,
          amount,
          pay_method: form.querySelector("#s-method").value,
          [isReceipt ? "received_at" : "paid_at"]: form.querySelector("#s-date").value,
  ...
  /* ---------- 手工新建应收/应付单弹窗 ---------- */
  function openCreateBill({ title, apiPath, parties, partyKey, partyLabel, onDone }) {
    const form = document.createElement("div");
    form.innerHTML = `
      <div class="field"><label>${partyLabel} *</label><select id="b-party"><option value="">请选择</option>
        ${parties.map((x) => `<option value="${x.id}">${UI.esc(x.name)}</option>`).join("")}</select></div>
      <div class="field"><label>金额 *</label><input type="number" min="0.01" step="0.01" id="b-amount" placeholder="大于 0"></div>
      <div class="field"><label>到期日</label><input type="date" id="b-due"></div>
      <div class="field"><label>备注</label><input type="text" id="b-remark" placeholder="选填"></div>`;
    const foot = document.createElement("div");
    foot.innerHTML = `<button class="btn btn-ghost" data-act="no">取消</button><button class="btn btn-primary" data-act="yes">创建</button>`;
    const m = UI.modal({ title, body: form, footer: foot, size: "modal-sm" });
    foot.querySelector('[data-act="no"]').onclick = m.close;
    foot.querySelector('[data-act="yes"]').onclick = async () => {
      const partyId = Number(form.querySelector("#b-party").value);
      const amount = Number(form.querySelector("#b-amount").value);
      if (!partyId) return UI.toast(`请选择${partyLabel}`, "error");
      if (!(amount > 0)) return UI.toast("金额必须大于 0", "error");
      try {
        await API.post(apiPath, {
          [partyKey]: partyId,
          amount,
          due_date: form.querySelector("#b-due").value || null,
          remark: form.querySelector("#b-remark").value,
        });
        UI.toast("创建成功");
        m.close();
        if (onDone) onDone();
  ...
  Pages.finance = {
    receivables: {
      render(el) {
        ...
        el.innerHTML = `
          <div class="card">
            <div class="toolbar">
              ...
              ${canManage ? `<span class="badge badge-cyan">应收余额合计 <b id="sum-balance">-</b></span><button class="btn btn-primary" id="add-receivable-btn">＋ 新建应收单</button>` : ""}
            </div>
        ...
        if (canManage) el.querySelector("#add-receivable-btn").onclick = async () => {
          const customers = await API.get("/api/master/customers/all");
          openCreateBill({ title: "新建应收单", apiPath: "/api/finance/receivables", parties: customers, partyKey: "customer_id", partyLabel: "客户", onDone: load });
        };
        ...
        async function load() {
          try {
            ...
            tbody.innerHTML = data.items.length ? data.items.map((r) => `<tr>
              ...
              <td class="actions">${canManage && r.status !== "settled" ? `<button class="btn-link" data-id="${r.id}">收款</button>` : '<span class="badge badge-gray">已核销</span>'}</td></tr>`).join("")
            ...
            tbody.querySelectorAll(".btn-link").forEach((b) => b.onclick = () => {
              ...
              if (!row) { UI.toast("数据已刷新，请重试", "error"); load(); return; }
    ...
    payables: {
      render(el) {
        ...
        el.innerHTML = `
          <div class="card">
            <div class="toolbar">
              ...
              <button class="btn btn-ghost" id="search-btn">查询</button>
              <div class="spacer"></div>
              ${canManage ? `<span class="badge badge-cyan">应付余额合计 <b id="sum-balance">-</b></span><button class="btn btn-primary" id="add-payable-btn">＋ 新建应付单</button>` : ""}
            </div>
            <div class="table-wrap"><table class="table">
        ...
        el.querySelector("#kw").onkeydown = (e) => { if (e.key === "Enter") el.querySelector("#search-btn").click(); };
        el.querySelector("#status").onchange = () => { state.status = el.querySelector("#status").value; state.page = 1; load(); };
        if (canManage) el.querySelector("#add-payable-btn").onclick = async () => {
          const suppliers = await API.get("/api/master/suppliers/all");
          openCreateBill({ title: "新建应付单", apiPath: "/api/finance/payables", parties: suppliers, partyKey: "supplier_id", partyLabel: "供应商", onDone: load });
        };
        ...
        async function load() {
          try {
            ...
            tbody.innerHTML = data.items.length ? data.items.map((p) => `<tr>
              ...
              <td class="actions">${canManage && p.status !== "settled" ? `<button class="btn-link" data-id="${p.id}">付款</button>` : '<span class="badge badge-gray">已核销</span>'}</td></tr>`).join("")
