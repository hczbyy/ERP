    ...
    checks: {
      render(el) {
        ...
        async function editItems(c) {
          ...
          form.innerHTML = d.items.map((it) => `
            <div class="field-row">
              <div class="field"><label>${UI.esc(it.product_name)}（账面 ${UI.qty(it.book_qty)}）</label>
                <input type="number" min="0" step="0.01" data-pid="${it.product_id}" value="${it.actual_qty}"></div>
            </div>`).join("");
          const foot = document.createElement("div");
          ...
          foot.querySelector('[data-act="no"]').onclick = m.close;
          foot.querySelector('[data-act="yes"]').onclick = async () => {
            const items = [...form.querySelectorAll("input[data-pid]")].map((i) => ({ product_id: Number(i.dataset.pid), actual_qty: Number(i.value) || 0 }));
        ...
        function openCreate() {
          (async () => {
            ...
            foot.querySelector('[data-act="yes"]').onclick = async () => {
              ...
              if (!items.length) return UI.toast("请至少添加一条调拨明细", "error");
              try {
